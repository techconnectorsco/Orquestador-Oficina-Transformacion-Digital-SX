<script lang="ts">
	...
	// File upload removed (Supabase Storage not available)
	// Use foto_url text input to provide an image URL directly

	function onFileChange(_e: Event) {
		uploadError = 'Carga de archivos no disponible. Usa una URL directa.';
	}

	async function subirFoto(): Promise<string | null> {
		return fFotoUrl || null;
	}
